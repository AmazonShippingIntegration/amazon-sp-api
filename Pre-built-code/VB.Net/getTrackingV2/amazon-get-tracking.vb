...
Public Class cls_LDV_AMZ
    ...
    Private IAMUserAccessKey As String = "Your Access Key Here"
    Private IAMUserSecretKey As String = "Your Secret Key Here"
    '*** La durata di refresh_token è annuale
    Private RefreshToken As String = "Your Refresh Token Here"

    Private LWAClinetId As String = "Your ClientId Here"
    Private LWAClientSecret As String = "Your Client Secret Here"
    ...
    Private Function SendRequest(ByVal url As System.Uri,
        ...
        Try
            ...
            If user <> "" Then
                ...
                request.Headers.Add("Authorization", "AWS4-HMAC-SHA256 Credential=$AWSAccessKeyHere/20220927/eu-west-1/execute-api/aws4_request, SignedHeaders=content-type;host;x-amz-access-token;x-amz-content-sha256;x-amz-date, Signature=331d734c58b5d98360bcaca6f4abf77042514df2df9758be59c07485c84f109b")
    ...
    Private Function get_Amz_Date() As String
        Dim dt1 As Date = DateAdd(DateInterval.Hour, -5, Now())
        Dim dt As Date = DateAdd(DateInterval.Minute, -30, dt1)
    ...
    Public Sub getTracking()
        Dim client = New RestClient("https:" & "//sellingpartnerapi-eu.amazon.com/shipping/v2/tracking?carrierId=AMZN_IT&trackingId=$trackingId")
        ...
        'https://sellingpartnerapi-eu.amazon.com/shipping/v2/tracking?carrierId=AMZN_IT&trackingId=IT2031368484
        ...
        canonicalHeaders &= "host:sellingpartnerapi-eu.amazon.com" & vbLf
        canonicalHeaders &= "x-amz-date:" & Amz_Date

        canonicalRequest = "GET" & vbLf                                  'HTTPRequestMethod 
        canonicalRequest &= "/shipping/v2/tracking" & vbLf         'CanonicalURI 
        canonicalRequest &= "carrierId=AMZN_IT&trackingId=IT2031368484" & vbLf
